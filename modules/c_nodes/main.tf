...
terraform {
  ...
  provider_meta "equinix" {
    module_name = "metal-mnmd/c-nodes"
  }
}
...
# create metal nodes
resource "equinix_metal_device" "c_nodes" {
  count            = var.node_count
  hostname         = format("c-%d", count.index + 2)
  plan             = var.plan
  metro            = var.metro
...
resource "equinix_metal_port" "port" {
  count = var.node_count
  port_id  = [for p in equinix_metal_device.c_nodes[count.index].ports : p.id if p.name == "bond0"][0]
  layer2   = false
  bonded   = true
  vlan_ids = var.metal_vlan.*.id
  depends_on = [equinix_metal_device.c_nodes]
}
